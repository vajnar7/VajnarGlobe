...
public class SharedPref
{
    ...
    static               SharedPreferences        defaultSharedPref        = null;
    static               SharedPreferences        defaultSharedPrefEncrypted = null;
    ...
    public SharedPref(Context context, boolean encrypted)
    {
        this(getPrefs(context, encrypted));
    }

    private static SharedPreferences getPrefs(Context context, boolean encrypted)
    {
        ...
        if (encrypted) {
            try {
                if (defaultSharedPrefEncrypted == null) {
                    MasterKey masterKey = new MasterKey.Builder(context).setKeyScheme(MasterKey.KeyScheme.AES256_GCM).build();
                    defaultSharedPrefEncrypted =
                            EncryptedSharedPreferences.create(context, "secret_shared_prefs", masterKey,
                                    EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV,
                                    EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM);
                }
                return defaultSharedPrefEncrypted;
            } catch (GeneralSecurityException | IOException ex) {
                ex.printStackTrace();
                return null;
            }
        } else
            return defaultSharedPref != null ?
                    defaultSharedPref : (defaultSharedPref = PreferenceManager.getDefaultSharedPreferences(context));
    }
    ...
    public static class PassEncryption
    {
        ...
        static String encode(String string)
        {
            ...
            byte[] dec        = Base64.encode(encodedStr.getBytes(), Base64.DEFAULT);
        ...
        static String decode(String string)
        {
            ...
            byte[]        dec           = new byte[0];
            try {
                dec = Base64.decode(string.getBytes("UTF-8"), Base64.DEFAULT);
